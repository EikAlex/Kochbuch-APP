...
def KAUFEN_ENDPOINT(eid): return f"{API_URL}/{eid}/kaufen"
    ...
    with st.expander("➕ Zutat zur Einkaufsliste hinzufügen"):
        try:
            ...
            menge = st.number_input(
                "Menge", min_value=1, step=1, key="menge_einkauf")
        ...
        if eintraege:
            for eintrag in eintraege:
                ...
                if col3.button("💾", key=f"save_{eintrag['id']}"):
                    payload = {"zutat_id": eintrag["id"], "menge": neue_menge}
                    res = requests.put(
                        f"{API_URL}/{eintrag['id']}", json=payload)
                    if res.ok:
                        st.success(
                            f"Menge von '{eintrag['zutat']}' aktualisiert!")
                        st.rerun()
                    else:
                        st.error(f"Fehler: {res.text}")
                ...
                if col4.button("🛍️", key=f"bought_{eintrag['id']}"):
                    ...
                    if res.ok:
                        st.success(
                            f"'{eintrag['zutat']}' als gekauft markiert und in Vorrat übertragen!")
                        st.rerun()
                    else:
                        st.error(f"Fehler: {res.text}")
